...
class MatchingConfig(BaseSettings):
    ...
    consumer_group: str = Field(
        "matching_service_group", alias="MATCHING_SERVICE_CONSUMER_GROUP"
    )
    consumer_name: str = Field(
        "matching_service_consumer",
        alias="MATCHING_SERVICE_CONSUMER_NAME",
    )
    max_parallel_tasks: int = Field(
        5,
        alias="MATCHING_SERVICE_MAX_PARALLEL_TASKS",
        ge=1,
        le=5,
    )
    read_block_ms: int = Field(5000, alias="MATCHING_SERVICE_READ_BLOCK_MS")
    error_sleep_seconds: int = Field(5, alias="MATCHING_SERVICE_ERROR_SLEEP_SECONDS")
    stale_task_idle_ms: int = Field(
        60000,
        alias="MATCHING_SERVICE_STALE_TASK_IDLE_MS",
        ge=1,
    )
    reclaim_batch_size: int = Field(
        5,
        alias="MATCHING_SERVICE_RECLAIM_BATCH_SIZE",
        ge=1,
        le=100,
    )
