...
class ResumeProcessingConfig(BaseSettings):
    """Конфигурация Resume Processing Service."""
    ...
    consumer_group: str = Field(
        "resume_processing_service_group", alias="RESUME_PROCESSING_CONSUMER_GROUP"
    )
    consumer_name: str = Field(
        "resume_processing_service_consumer",
        alias="RESUME_PROCESSING_SERVICE_CONSUMER_NAME",
    )
    max_parallel_tasks: int = Field(
        5,
        alias="RESUME_PROCESSING_SERVICE_MAX_PARALLEL_TASKS",
        ge=1,
        le=5,
    )
    read_block_ms: int = Field(5000, alias="RESUME_PROCESSING_SERVICE_READ_BLOCK_MS")
    error_sleep_seconds: int = Field(
        5, alias="RESUME_PROCESSING_SERVICE_ERROR_SLEEP_SECONDS"
    )
    stale_task_idle_ms: int = Field(
        60000,
        alias="RESUME_PROCESSING_SERVICE_STALE_TASK_IDLE_MS",
        ge=1,
    )
    reclaim_batch_size: int = Field(
        5,
        alias="RESUME_PROCESSING_SERVICE_RECLAIM_BATCH_SIZE",
        ge=1,
        le=100,
    )
    max_delivery_attempts: int = Field(
        3,
        alias="RESUME_PROCESSING_SERVICE_MAX_DELIVERY_ATTEMPTS",
        ge=1,
    )
...
class ServiceConfig(BaseSettings):
    ...
    resume_processing: ResumeProcessingConfig = ResumeProcessingConfig()
